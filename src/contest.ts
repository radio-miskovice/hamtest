...
export async function addContest(argv: string[]) {
  if (argv.length < 3)
  ...
  if (!end.match(patternIsoDatetime)) {
    ...
    else throw (`'add contest' end parameter format is invalid, must be yyyy-mm-ddThh:mm[:ss][z]`);
  }
  ...
  try {
    ...
    else console.log(`${e} in add contest '${id}'`);
  }
  finally {
    await db.end();
  }
