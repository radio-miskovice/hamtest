...
class CabrilloObject {
     ...
     */
    static frequencyToBand(freq) {
        for (let i = 0; i < BANDS.length; i++) {
            if (freq >= FREQUENCY[2 * i] && freq <= FREQUENCY[2 * i + 1])
                return BANDS[i];
        }
        return '';
    }
                ...
                for (let i = 4, j = 0; i < line.length; i++) {
                    if (line[i] == ' ')
                        ...
                        if (t < 0) {
                            ...
                            if (t >= 0)
                                template = [...(template.slice(0, t)), ...(template.slice(t + 1))];
                        }
                        j = 0;
                    }
                }
        ...
        for (let qsoRec of this.qsoArray) {
            for (let i = 0; i < this.templateMask.length && i < qsoRec.length; i++) {
                ...
                // check for integer number
                    ...
                    if (i == 0 && ti.type == null)
                        ti.type = 'F'; // the first field is frequency
                    else if (ti.type == null || 'RT'.match(ti.type))
                        ti.type = 'N'; // previously recognized as R or T is in fact not that
                }
            ...
            if (line.match(/^QSO:/)) {
                ...
                for (let i = 0; i < this.templateMask.length; i++) {
                    if (i + 1 == this.templateMask.length) {
                        ...
                        f = line.slice(this.templateMask[i].start, this.templateMask[i + 1].start);
    ...
    setTemplateArray(templateArray, ignoreUnusedFields = true) {
        if (templateArray.length > this.templateMask.length)
            throw "Template array has more elements than the detected template mask";
        if (!ignoreUnusedFields && (templateArray.length < (this.templateMask.length - 4)))
            throw "Template array has more elements than the detected template mask";
    ...
    convertQsoRecord2Data(qso) {
        ...
        let data = { qso_band, qso_mode, qso_time, calls: '', callr: '' };
        for (let key of this.field.keys()) {
            const index = this.field.get(key);
            if (key.match(/^(nrr|nrs)$/))
                data[key] = parseInt(qso[index]);
            else
                data[key] = qso[index];
        }
        return data;
...
exports.default = CabrilloObject;
;
//# sourceMappingURL=cabrillo.js.map